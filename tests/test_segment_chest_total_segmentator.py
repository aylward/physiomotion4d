...
class TestSegmentChestTotalSegmentator:
    ...
    def test_segmenter_initialization(self, segmenter_total_segmentator):
        ...
        assert segmenter_total_segmentator.target_spacing == 1.5, "Target spacing not set correctly"
        
        # Check that anatomical structure ID mappings are defined
        assert len(segmenter_total_segmentator.heart_mask_ids) > 0, "Heart mask IDs not defined"
        assert len(segmenter_total_segmentator.major_vessels_mask_ids) > 0, "Major vessels mask IDs not defined"
        assert len(segmenter_total_segmentator.lung_mask_ids) > 0, "Lung mask IDs not defined"
        assert len(segmenter_total_segmentator.bone_mask_ids) > 0, "Bone mask IDs not defined"
        assert len(segmenter_total_segmentator.soft_tissue_mask_ids) > 0, "Soft tissue mask IDs not defined"
        
        print("\n✓ Segmenter initialized with correct parameters")
        print(f"  Heart structures: {len(segmenter_total_segmentator.heart_mask_ids)}")
        print(f"  Major vessels: {len(segmenter_total_segmentator.major_vessels_mask_ids)}")
        ...
        print(f"  Soft tissue structures: {len(segmenter_total_segmentator.soft_tissue_mask_ids)}")

    def test_segment_single_image(self, segmenter_total_segmentator, test_images, test_directories):
        ...
        # Run segmentation
        result = segmenter_total_segmentator.segment(input_image, contrast_enhanced_study=False)
    ...
    def test_segment_multiple_images(self, segmenter_total_segmentator, test_images, test_directories):
    ...
    def test_contrast_detection(self, segmenter_total_segmentator, test_images):
        ...
        result_no_contrast = segmenter_total_segmentator.segment(input_image, contrast_enhanced_study=False)
        contrast_mask_no = result_no_contrast["contrast"]
        
        # Test with contrast flag
        result_with_contrast = segmenter_total_segmentator.segment(input_image, contrast_enhanced_study=True)
